...
<a class="skip-link sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[100] focus:bg-primary focus:text-white focus:px-6 focus:py-3 focus:rounded-lg focus:font-bold" href="#main-content">
    <?php esc_html_e( 'Skip to main content', 'hasooleilm-lms' ); ?>
</a>

<!-- Removed 'site-header' class -->
<header id="masthead" class="sticky top-0 z-50 w-full bg-white shadow-sm transition-shadow duration-300" role="banner" aria-label="<?php esc_attr_e( 'Site Header', 'hasooleilm-lms' ); ?>">
    <div class="max-w-[1200px] mx-auto px-5 lg:px-10 h-[70px] flex items-center justify-between">
        
        <!-- Logo Area: Removed 'site-branding' class -->
        <div class="flex items-center gap-3">
            <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="flex items-center gap-3 text-gray-900 no-underline hover:opacity-80 transition-opacity" aria-label="<?php esc_attr_e( 'Hasooleilm LMS - Home', 'hasooleilm-lms' ); ?>">
                ...
                <?php else : ?>
                    <img src="<?php echo esc_url( get_template_directory_uri() . '/assets/images/logo.png' ); ?>" 
                         ...
                         class="w-[45px] h-[45px] md:w-[50px] md:h-[50px] lg:w-[60px] lg:h-[60px] object-contain rounded-lg" 
                         width="60" 
                         height="60">
                <?php endif; ?>
            ...
            ?>
                <p class="hidden md:block text-sm text-gray-600 mt-1"><?php echo esc_html( $description ); ?></p>
            <?php endif; ?>
        </div>

        <!-- Mobile Menu Toggle -->
        <button class="mobile-menu-toggle md:hidden flex items-center justify-center w-11 h-11 bg-transparent border-2 border-gray-200 rounded-lg cursor-pointer p-0 hover:bg-secondary hover:border-primary transition-all" 
                aria-expanded="false" 
        ...
        <!-- Primary Navigation: Removed 'primary-navigation' class -->
        <nav id="primary-navigation" class="hidden md:flex absolute md:static top-[70px] left-0 right-0 bg-white md:bg-transparent shadow-lg md:shadow-none p-5 md:p-0 z-[999] md:z-auto items-center gap-2" role="navigation" aria-label="<?php esc_attr_e( 'Primary Navigation', 'hasooleilm-lms' ); ?>">
            <?php
            wp_nav_menu( array(
            ...
            <!-- Search Bar: Removed 'header-search' class -->
            <div class="mt-5 md:mt-0 pt-5 md:pt-0 border-t border-gray-200 md:border-0 md:ml-6">
                <form role="search" method="get" class="flex gap-2 md:relative" action="<?php echo esc_url( home_url( '/' ) ); ?>" aria-label="<?php esc_attr_e( 'Search site', 'hasooleilm-lms' ); ?>">
                    ...
                    <button type="submit" class="flex items-center justify-center w-11 h-11 md:w-9 md:h-9 bg-primary border-none rounded-lg text-white cursor-pointer hover:bg-primary-dark transition-colors absolute md:absolute right-1 top-1/2 -translate-y-1/2" aria-label="<?php esc_attr_e( 'Submit search', 'hasooleilm-lms' ); ?>">
