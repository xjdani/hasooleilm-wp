...
<footer id="colophon" class="bg-primary-dark text-white mt-16" role="contentinfo" aria-label="<?php esc_attr_e( 'Site Footer', 'hasooleilm-lms' ); ?>">

    <div class="py-12 md:py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid gap-8 md:gap-12 grid-cols-1 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1.5fr]">

                <div class="max-w-[350px] lg:max-w-none">
                    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="flex items-center gap-3 text-white no-underline hover:opacity-85 transition-opacity mb-4" aria-label="<?php esc_attr_e( 'Hasooleilm LMS - Home', 'hasooleilm-lms' ); ?>">
                        <?php if ( has_custom_logo() ) : ?>
                            <?php the_custom_logo(); ?>
                        <?php else : ?>
                            <img src="<?php echo esc_url( get_template_directory_uri() . '/assets/images/logo.png' ); ?>"
                                 alt="<?php esc_attr_e( 'Hasooleilm LMS Logo', 'hasooleilm-lms' ); ?>"
                                 class="h-[50px] w-auto object-contain rounded-lg"
                                 width="50"
                                 height="50">
                        <?php endif; ?>
                        <span class="text-xl md:text-2xl font-bold"><?php bloginfo( 'name' ); ?></span>
                    </a>
                    <p class="text-sm md:text-base opacity-90 leading-relaxed mb-0">
                ...
                <div>
                    <h3 class="text-base md:text-lg font-bold mb-4 tracking-wide"><?php esc_html_e( 'Study', 'hasooleilm-lms' ); ?></h3>
                ...
                <div>
                    <h3 class="text-base md:text-lg font-bold mb-4 tracking-wide"><?php esc_html_e( 'Resources', 'hasooleilm-lms' ); ?></h3>
                    <ul class="list-none p-0 m-0 space-y-2.5">
                ...
                <div class="flex flex-col gap-8">
                    <div>
                        <h3 class="text-base md:text-lg font-bold mb-4 tracking-wide"><?php esc_html_e( 'Contact', 'hasooleilm-lms' ); ?></h3>
                    ...
                    <div>
                        <h3 class="text-base md:text-lg font-bold mb-4 tracking-wide"><?php esc_html_e( 'Stay Updated', 'hasooleilm-lms' ); ?></h3>
                        <p class="text-xs md:text-sm opacity-85 mb-4 leading-relaxed">
                            <?php esc_html_e( 'New past papers, SLO updates, and date sheet changes -- straight to your inbox.', 'hasooleilm-lms' ); ?>
                        </p>
                        <form class="flex gap-2 flex-wrap" action="#" method="post" aria-label="<?php esc_attr_e( 'Newsletter subscription', 'hasooleilm-lms' ); ?>">
                            <label for="newsletter-email" class="sr-only"><?php esc_html_e( 'Your email address', 'hasooleilm-lms' ); ?></label>
                            <input type="email"
                                   id="newsletter-email"
                                   class="flex-1 min-w-[200px] px-3.5 py-2.5 border-2 border-white/20 rounded-lg bg-white/10 text-white font-sans text-sm md:text-base placeholder:text-white/60 focus:outline-none focus:border-secondary focus:bg-white/15 transition-all"
                                   placeholder="<?php esc_attr_e( 'Your email', 'hasooleilm-lms' ); ?>"
                                   required
                                   aria-required="true">
                            <button type="submit" class="px-5 py-2.5 bg-secondary text-primary-dark border-none rounded-lg font-sans text-sm md:text-base font-semibold cursor-pointer hover:bg-white hover:-translate-y-0.5 transition-all whitespace-nowrap" aria-label="<?php esc_attr_e( 'Subscribe to newsletter', 'hasooleilm-lms' ); ?>">
    ...
    <div class="border-t border-white/15 py-6 bg-primary-dark">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row gap-4 items-center text-center sm:text-left sm:justify-between">
                <p class="text-xs md:text-sm opacity-80 m-0">
                    &copy; <?php echo date('Y'); ?> Hasooleilm.
                    <?php esc_html_e( 'All rights reserved.', 'hasooleilm-lms' ); ?>
                </p>
                <nav class="flex gap-6 flex-wrap justify-center sm:justify-end" aria-label="<?php esc_attr_e( 'Legal links', 'hasooleilm-lms' ); ?>">
